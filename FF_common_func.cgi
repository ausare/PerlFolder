...
sub convertQuotes { 
	...
	my $problems;
		...
		foreach my $paraGraph (@allrecords) { #switch every other html " (&quot;) with the `` or ''
			...
			while ($returnText =~ m/\"/g) { #as long as there's a html quote replace them. 
 		 		$returnText =~ s/\"/``/;
 		 		$returnText =~ s/\"/\'\'/;
			}
			if ($returnText =~ m/\s\'\'/g) { #as long as there's a html quote replace them. 
 		 		$returnText =~ s/\s\'\'/ \`\`/g;
			}
			if ($returnText =~ m/\`\`\s/g) { #as long as there's a html quote replace them. 
 		 		$returnText =~ s/\`\`\s/\'\' /g;
			}
			if ($returnText =~ m/\n\'\'/g) { #as long as there's a html quote replace them. 
 		 		$returnText =~ s/\n\'\'/\n\`\`/g;
			}
			#if ($returnText =~ m/\n\<strong\>\'\'/g) { #as long as there's a html quote replace them. 
 		 	#	$returnText =~ s/\n\<strong\>\'\'/\n\<strong\>\`\`/g;
			#}
...
print "	</ul>\n";
	...
	#	print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=\"Photo Poster\"&pageType=archives\">Photo Poster &rarr;</a>\n";
	#		print "	<ul>\n";
	#			print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=Photo Poster&pageType=archives&weekof=0\">$allbaseweeks[5]</a>\n";
	#			print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=Photo Poster&pageType=archives&weekof=-1\">$allbaseweeks[4]</a>\n";
	#			print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=Photo Poster&pageType=archives&weekof=1\">$allbaseweeks[6]</a>\n";
	#		print "	</ul>\n";			
...
######################
#foreach (@allGroups) {
#my @setPieces = split('\|',$_);
#	if ($setPieces[0] ne "SetName" ){
#		print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageType=photoSets&pageMode=view&feature=$setPieces[0]\">$setPieces[0]</a>\n";
#	}
#	print "	<li><a href=\"#\">Display photo sets &darr;</a>\n";
#	print "	<ul>\n";
#		foreach my $set (@allGroups) {
#			my @setPieces = split('\|',$set);
#			print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=$setPieces[0]&pageType=sets\">$setPieces[0]   &rarr;</a>\n";
...
#print "	</ul>\n";
	...
	#print "	<ul>\n";
	#	foreach my $weeklink (@allbaseweeks) {
	#		print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageMode=bulkUpload&weekof=$weeklink\">$weeklink</a>\n";
	#	}
		print "</li>\n";
...
# new drop goes here
#print "	<li><a href=\"http://$ThisIP/cgi-bin/FE2/featurePhotos.cgi?pageType=photoSets&pageMode=choose\">Run Photo Sets</a>\n";
#my @setPieces = split('\|',$_);
#print "	<ul>\n";
...
sub makeaPhotodir {
	...
	unless(-e $scanDir or mkdir $scanDir) {
		print  "Uh-oh! Scans Volume is not mounted on this machine. Please contact lgonyea\@gracenote.com\n";
	}
	chmod(0777,$scanDir);
...
if ($thisstatus eq 'posted') {
	...
	$toLine = "lgonyea\@gracenote.com,jagifford\@gracenote.com";
}		
	my $runninglog = "";
	my $mailline = "";
  ...
  #	$toLine = "lrgonyea\@gracenote.com, gdickie\@gracenote.com";
    ...
    From    => "TESTFeaturesWriter\@gracenote.com",
    To      => "$toLine",
    Subject => "$subjectLine",
